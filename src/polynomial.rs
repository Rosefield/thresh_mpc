...
pub fn lagrange_poly<T: Field + Copy, F: Fn(&T) -> T>(points: &[T], xi: &T, num_func: F) -> T {
    let (n, d) = points
        .iter()
        .filter(|xj| xi != *xj)
        .map(|xj| (num_func(xj), (*xi - xj)))
...
impl<T: Field + Copy> Polynomial<T> for InterpolationPolynomial<T> {
    fn evaluate(&self, x: &T) -> T {
        ...
        self.points
            ...
            .zip(self.vals.iter())
            .map(|(xi, yi)| {
                let l = lagrange_poly(&self.points, xi, |xj| *x - xj);
                return l * yi;
            })
            .sum()
    }
    ...
    fn eval_zero(&self) -> T {
        self.points
            ...
            .zip(self.vals.iter())
            .map(|(xi, yi)| {
                let l = lagrange_poly(&self.points, xi, |x| *x);
                return l * yi;
            })
            .sum()
    }
